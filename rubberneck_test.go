...
var _ = Describe("Rubberneck", func() {
	...
	Describe("NewPrinterWithKeyMasking()", func() {
		...
		It("returns a properly configured Printer", func() {
			...
			Expect(printer.MaskWithValue).To(BeNil())
	...
	Describe("NewPrinterWithKeyValueMasking()", func() {
		var didPrint bool
		var didMask bool

		var printFunc func(format string, v ...interface{})
		var maskFunc func(argument string, val interface{}) *string
		var printable struct{ Content string }
		var printedValue = ""
		...
		It("returns a properly configured Printer", func() {
			printer := NewPrinterWithKeyValueMasking(printFunc, maskFunc, AddLineFeed)
			Expect(printer.Show).NotTo(BeNil())
			Expect(printer.MaskWithValue).NotTo(BeNil())
			Expect(printer.Mask).To(BeNil())
			...
			Expect(didMask).To(BeTrue())
			Expect(printedValue).NotTo(HaveLen(0))
			Expect(printedValue).To(ContainSubstring("grendel MASKED"))
		})
	})
